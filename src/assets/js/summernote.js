    ...
    $$1.extend($$1.summernote.lang, {
        'en-US': {
            ...
            link: {
                ...
                urls :'Paste your embed code below:',
    ...
    */
    ...
    var Editor = /** @class */ (function () {
        ...
        Editor.prototype.onFormatBlock = function (tagName, $target) {
            ...
            if(tagName === 'center'){
                document.execCommand('justifyCenter', false, tagName);
            }else{
        ...
        Buttons.prototype.addToolbarButtons = function () {
            ...
            this.context.memo('button.links', function () {
                return _this.button({
                    ...
                    tooltip: _this.lang.link.embed + _this.representShortcut('linkDialogs.show'),
                    click: _this.context.createInvokeHandler('linkDialogs.show')
                }).render();
            });
            ...
            this.context.memo('button.left', function () {
                return _this.button({
                    contents: _this.ui.icon(_this.options.icons.alignLeft),
                    tooltip: _this.lang.left.left+ _this.representShortcut('justifyLeft'),
                    click: _this.context.createInvokeHandler('editor.justifyLeft')
                }).render();
            });
            this.context.memo('button.center', function () {
                return _this.button({
                    contents: _this.ui.icon(_this.options.icons.alignCenter),
                    tooltip: _this.lang.center.center + _this.representShortcut('justifyCenter'),
                    click: _this.context.createInvokeHandler('editor.justifyCenter')
                }).render();
            });
            this.context.memo('button.outdent', function () {
                return _this.button({
                    ...
                    tooltip: _this.lang.outdent.outdent + _this.representShortcut('outdent'),
            ...
            this.context.memo('button.indent', function () {
                return _this.button({
                    ...
                    tooltip: _this.lang.indent.indent + _this.representShortcut('indent'),
...
// Start Embed content
    var LinkDialogs = /** @class */ (function () {
        function LinkDialogs(context) {
            this.context = context;
            this.ui = $$1.summernote.ui;
            this.$body = $$1(document.body);
            this.$editor = context.layoutInfo.editor;
            this.options = context.options;
            this.lang = this.options.langInfo;
            context.memo('help.linkDialogs.show', this.options.langInfo.help['linkDialogs.show']);
        }
        LinkDialogs.prototype.initialize = function () {
            var $container = this.options.dialogsInBody ? this.$body : this.$editor;
            var body = [
                // '<div class="form-group note-form-group">',
                // "<label class=\"note-form-label\">" + this.lang.link.textToDisplay + "</label>",
            ...
            this.$dialog = this.ui.dialog({
                className: 'link-dialog',
                title: this.lang.link.inserts,
                fade: this.options.dialogsFade,
                body: body,
                footer: footer
            }).render().appendTo($container);
        };
        LinkDialogs.prototype.destroy = function () {
            this.ui.hideDialog(this.$dialog);
            this.$dialog.remove();
        };
        LinkDialogs.prototype.bindEnterKey = function ($input, $btn) {
            $input.on('keypress', function (event) {
                if (event.keyCode === key.code.ENTER) {
                    event.preventDefault();
                    $btn.trigger('click');
         ...
         * toggle update button
         */
        LinkDialogs.prototype.toggleLinkBtn = function ($linkBtn, $linkText, $linkUrl) {
            this.ui.toggleBtn($linkBtn, $linkText.val() && $linkUrl.val());
        };
        /**
         * Show link dialog and set event handlers on dialog controls.
         ...
         * @return {Promise}
         */
        LinkDialogs.prototype.showLinkDialogs = function (linkInfo) {
            var _this = this;
            return $$1.Deferred(function (deferred) {
                var $linkText = _this.$dialog.find('.note-link-url');
                var $linkUrl = _this.$dialog.find('.note-link-url');
                var $linkBtn = _this.$dialog.find('.note-link-btn');
                var $openInNewWindow = _this.$dialog.find('input[type=checkbox]');
                _this.ui.onDialogShown(_this.$dialog, function () {
         ...
         * @param {Object} layoutInfo
         */
        LinkDialogs.prototype.show = function () {
            var _this = this;
            var linkInfo = this.context.invoke('editor.getLinkInfo');
            this.context.invoke('editor.saveRange');
            this.showLinkDialogs(linkInfo).then(function (linkInfo) {
                _this.context.invoke('editor.restoreRange');
                _this.context.invoke('editor.createLink', linkInfo);
            }).fail(function () {
                _this.context.invoke('editor.restoreRange');
...
// end Embed content
    ...
    $$1.summernote = $$1.extend($$1.summernote, {
        ...
        options: {
            ...
            container: 'body',
            maxTextLength: 0,
            styleTags: ['p', 'blockquote', 'pre', 'h3','center'],
            fontNames: [
