...
export class TpacompaniesListComponent implements OnInit {
  ...
  constructor(
    ...
    this.meta.addTag({ name: 'description', content: 'All the list of Analysts' });
    this.meta.addTag({ name: 'author', content: ENV.AUTHOR });
    this.meta.addTag({ name: 'keywords', content: 'clients, revere, equity' });
  }

  ngOnInit(): void {
    this.bcList = [{ label: 'Home', url: 'home', params: [] }, { label: 'Analysts', url: 'analysts', params: [] }];
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        var myEfficientFn = this._utils.debounce(() => {
          let apiEvent = this._userApi.filterUsers$(dataTablesParameters, 'filterAnalysts')
            .subscribe(resp => {
  ...
  download() {
    this._userApi.getClient$()
      .subscribe(data => {
        ...
        new Angular2Csv(this.allItems, 'AnalystsList', options);
      });
  }
